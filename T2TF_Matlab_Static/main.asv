...
% Vehicle 1
gospaLocal.vec1 = initilizeGospa(truthGospa);
...
gospaFused.vec1 = initilizeGospa(truthGospa);
...
% Vehicle 2
gospaLocal.vec2 = initilizeGospa(truthGospa);
...
gospaFused.vec2 = initilizeGospa(truthGospa);
...
% Vehicle 3
gospaLocal.vec3 = initilizeGospa(truthGospa);
...
gospaFused.vec3 = initilizeGospa(truthGospa);
   ...
   %}
    ...
    if wasTracker1Updated || wasFuser2Updated
        ...
        if isLocked(v1Fuser) || ~isempty(tracksToFuse1)
            ...
            plotTrack(plotters.veh1FusePlotter,pos,ids); %Display fused track "square marker"
        else
            wasFuser1Updated = false;
            fusedTracks1 = objectTrack.empty(0,1);
        end
