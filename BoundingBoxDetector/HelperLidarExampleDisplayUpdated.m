classdef HelperLidarExampleDisplayUpdated < matlab.System
    ...
    methods
        % Constructor
        function obj = HelperLidarExampleDisplayUpdated(varargin)
            % Create each display
            if mod(numel(varargin),2) == 1
                ...
                inargs = varargin(1:end);
            end
            setProperties(obj,numel(inargs), inargs{:});
        end
        ...
        function writeAnimatedGIF(obj,frame1,frame2,fName,type)
            switch lower(type(1))
                case 'a' % all figures
                    assert(~isempty(obj.MovieName),'Must specify movie name');
                    allFrames = obj.pVideoFrames(frame1:frame2);
    ...
    methods (Access = protected)
        function setupImpl(obj,~,confTracks,lidarScan,~,...
                ~,~,~,varargin)
           %obj,detections,confTracks,lidarScan,onbstacleIndices,groundIndices,...
            % croppedIndices,currentImage,varargin) %#ok<INUSL>
            if obj.Figure == 0
                createPreProcessingDisplay(obj);
                createTrackingAnalysisDisplay(obj,currentImage);
                createCloseUpDisplay(obj); 
            end
        ...
        function stepImpl(obj,~,confTracks,lidarScan,~,... 
                ~,~,~,varargin)
            %obj,detections,confTracks,lidarScan,obstacleIndices,...
        ...
        function createPreProcessingDisplay(obj)
            f = figure('Units','normalized','OuterPosition',[0.1 0.1 0.8 0.8],'InvertHardcopy','off','Visible','off',...
                'Tag','lidarTrackingExampleDisplay','CloseRequestFcn',@(x,~)set(x,'Visible','off'));
            ...
            ax = axes('Parent',p1);
            obj.pPCAxes = ax;
            set(ax,'XLim',[-60 60],'YLim',[-10 20],'ZLim',[-2 15]);
            ax.NextPlot = 'add';
            ax.Color = [0 0 0];
            grid(ax,'on');
...
for i = 1:numel(length)
    ...
    detOrientations(:,:,i) = rotmat(quaternion([yaw(i) 0 0],'eulerd','ZYX','frame'),'frame');
end
else
    pos = zeros(0,3);
