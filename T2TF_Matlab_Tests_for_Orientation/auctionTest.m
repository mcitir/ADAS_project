function [rowSoln, colSoln, colRedux] = ...
    auctionTest(costMatrix, varargin)
...
while ~lastPhase
    ...
    while any(isnan(rowSoln)) && ~(lastPhase && auctionCount >= maxAuctions)
        ...
        % Forward auction
        disp(['forward auction: ', num2str(auctionCount), ': bids from local tracks'])
        ...
        % Reverse auction
        disp(['reverse auction: ', num2str(auctionCount), ': bids from central tracks'])
...
while sum(~isUnassigned) < (nAssigned+1) && priceWarIter < 1000
    ...
    for thisRow = 1:numel(rowList)
        disp(['to target ', num2str(thisRow)])
